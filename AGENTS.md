...
- 测试名称应能表达被测行为和具体场景。
- 在本仓库运行 Python 相关命令时，优先使用 `.venv/bin/python`、`.venv/bin/pytest` 和 `.venv/bin/pip`。
- 如果 `.venv` 不存在或缺少测试依赖，应先明确说明阻塞点，再考虑其他执行方式。
- 在执行测试前，优先确认虚拟环境里已经安装了所需测试依赖，避免把环境问题误判为代码问题。
