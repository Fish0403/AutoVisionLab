...
为支持后续半自治调优，Run 除基础信息外建议显式维护 3 个锚点：

- `baseline_experiment_id`
  当前 run 的基线实验，作为 proposal、reflection 和趋势比较的固定参照
- `best_experiment_id`
  当前 run 下综合排序后的最佳实验，供结果区默认展示和自动调优收敛判断
- `frontier_experiment_id`
  当前继续向前扩展的实验分支起点，不一定等于 best，但代表下一轮探索的默认来源

这样做的优势：

- proposal、reflection 和前端展示都有稳定参照，不再默认依赖“最新实验”
- 可以区分“当前最好”和“当前继续探索的分支”，避免搜索策略被最新一次试验绑死
- 为后续暂停、恢复、人工审批和多分支探索预留清晰状态锚点
...
除训练状态外，Experiment 建议增加一层“研究决策状态”：
...
- 把“跑过”与“值得保留”分开，避免仅靠 `success` 混淆实验价值
- crash、timeout、人工终止、效果退化都能被结构化记录，便于后续分析
- 前端可以直接展示哪些实验只是尝试，哪些实验真正推进了 run
...
在当前“参数提案驱动”的框架不变前提下，下一阶段优先补这 3 个能力。

#### A. Run 显式锚点

每个 run 显式维护 `baseline`、`best`、`frontier` 三类实验指针。

适用原因：

- 当前平台已经支持连续追加实验，但缺少研究语义上的稳定锚点
- 当自动调优轮数增多时，只看最新实验不够，容易让展示与搜索策略都漂移

#### B. Experiment 决策层

在训练状态之外，再增加“研究决策”这一层。

建议区分：

- 训练状态：`queued`、`running`、`success`、`failed`、`discarded`
- 决策状态：`keep`、`discard`、`crash`、`timeout`

适用原因：

- `success` 只代表跑完，不代表值得继续沿用
- `discarded` 目前更多表示人为中断，不足以覆盖 crash 和 timeout 等研究语义
- 自动调优后续要做统计、回放、筛选时，需要看的是决策结果而不只是执行状态

#### C. 统一排名与保留策略

在固定模型结构、固定训练流程的前提下，当前阶段继续以固定 `epochs` 作为主比较方式，不额外引入固定时长预算。

建议支持：

- 明确标记该实验是否参与 best 排名
- 明确 best 的综合排序规则
- 明确 keep / discard 的自动更新规则

这样做的优势：

- 实现更简单，符合当前平台“固定模型、只调参数”的约束
- 直接沿用现有 epoch 语义，减少用户理解成本
- 先把 best 排名和决策状态做稳，再考虑是否需要更复杂的预算模式
